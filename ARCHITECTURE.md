...
data: a FastAPI + React app on Cloud Run that reads a customer's accounts,
...
3. [Agentic Workflow](Agentic-Workflow.md) — the 3 LLM agents, the
...
| Agents | `cloud-run/edrak/app/agents/` | transaction_intelligence, decision_advisor, intervention (+ gemini_client, schemas) |
| Data | `cloud-run/edrak/app/data/` | bigquery_client (all reads/writes), seed generator/loader |
| UI | `cloud-run/edrak/ui/` | React: mode select, forecast chart, bank panel, radar |
