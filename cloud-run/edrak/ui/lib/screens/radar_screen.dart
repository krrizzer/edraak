...
class _RadarScreenState extends State<RadarScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('الرادار المالي', style: TextStyle(fontWeight: FontWeight.w900)),
      ),
      body: ListView(
        ...
        children: [
          SectionCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              PrimaryButton('محاكاة فحص نهاية الشهر', loading: _loading, onPressed: _trigger),
          ...
          if (_loading) LoadingPanel(title: 'جاري فحص مسار الشهر الحالي', steps: _steps),
...
class _RadarResult extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cuts = ((trajectory['suggested_cuts'] ?? []) as List).cast<Map<String, dynamic>>();
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Icon(isAlert ? Icons.warning_amber_rounded : Icons.verified_outlined,
                color: color, size: 26),
          ...
          Text(result['message_ar'] as String, style: const TextStyle(fontSize: 15.5, height: 1.9)),
          const SizedBox(height: 12),
          Wrap(spacing: 8, runSpacing: 8, children: [
            if (isAlert) ...[
              ...
              if (cause != null)
                Pill('السبب: ${cause['label_ar']} (+${cause['deviation_pct']}%)', color: color),
            ] else
              Pill('الرصيد المتوقع نهاية الشهر: ${sar(trajectory['projected_eom_balance'])}',
                  color: AppColors.primary),
          ]),
          if (cuts.isNotEmpty) ...[
            const SizedBox(height: 12),
            const Text('أسرع الحلول:', style: TextStyle(fontWeight: FontWeight.w800)),
...
class _BudgetCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SectionCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: Metric('قابل للصرف الآن', sar(trajectory['balance_now']))),
          const SizedBox(width: 10),
          Expanded(child: Metric('مدخرات (محمية)', sar(trajectory['savings_reserve']),
        ...
        const Text('الميزانية محسوبة من متوسط إنفاقك والتزاماتك — المدخرات لا تُحتسب ضمن الصرف.',
...
class _Trajectory extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final categories = (trajectory['categories'] as List).cast<Map<String, dynamic>>();
    final payments = (trajectory['upcoming_payments'] as List).cast<Map<String, dynamic>>();
    final trough = trajectory['projected_trough'] as Map<String, dynamic>?;
    return SectionCard(
      title: 'الأرقام خلف النتيجة',
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Wrap(spacing: 8, runSpacing: 8, children: [
          Pill('الصرف اليومي: ${sar(trajectory['daily_flexible_pace'])}', color: AppColors.accent),
          Pill('المتوقع نهاية الشهر: ${sar(trajectory['projected_eom_balance'])}', color: AppColors.accent),
          if (trough != null && (trough['amount'] as num) < 0)
            Pill('أدنى نقطة: ${sar(trough['amount'])} في ${trough['date']}', color: AppColors.danger),
          if (trajectory['expected_salary_day'] != null)
            Pill('الراتب متوقع يوم ${trajectory['expected_salary_day']}', color: AppColors.accent),
        ]),
        const SizedBox(height: 16),
        const Text('وتيرة الصرف حسب الفئة', style: TextStyle(fontWeight: FontWeight.w800)),
        const SizedBox(height: 8),
        ...categories.take(6).map((c) {
          final dev = (c['deviation_pct'] as num).toInt();
          final devColor = dev > 20 ? AppColors.danger : (dev < -20 ? AppColors.primary : AppColors.textMuted);
          return Padding(
            padding: const EdgeInsets.symmetric(vertical: 5),
            child: Row(children: [
              Expanded(flex: 3, child: Text('${c['label_ar']}')),
              Expanded(flex: 2, child: Text(sar(c['mtd']),
                  style: const TextStyle(color: AppColors.textMuted))),
              Expanded(flex: 2, child: Text(sar(c['baseline_mtd']),
                  style: const TextStyle(color: AppColors.textMuted))),
              SizedBox(width: 56, child: Text('${dev > 0 ? '+' : ''}$dev%',
                  textAlign: TextAlign.end,
                  style: TextStyle(color: devColor, fontWeight: FontWeight.w900))),
            ]),
          );
        }),
        if (payments.isNotEmpty) ...[
          const SizedBox(height: 14),
          const Text('الالتزامات القادمة هذا الشهر', style: TextStyle(fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          ...payments.map((p) => Padding(
                padding: const EdgeInsets.symmetric(vertical: 3),
                child: Text('يوم ${p['day']}: ${p['label']} — ${sar(p['amount'])}',
                    style: const TextStyle(color: AppColors.textMuted)),
              )),
        ],
      ]),
          ...
          return Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                Pill(isOverspend ? 'تجاوز ميزانية' : 'قسط في خطر', color: color),
              ]),
              const SizedBox(height: 6),
              Text('${a['message_ar']}',
